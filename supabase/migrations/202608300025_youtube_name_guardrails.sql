...
begin
  ...
  for participant in select value from jsonb_array_elements(
    ...
    if nullif(trim(participant->>'personId'), '') is null
      and coalesce(participant->>'matchStatus', 'new') in ('new', 'review') then
      continue;
    end if;
...
comment on function public.sync_segment_appearance() is
  'Sincroniza solo personas confirmadas; las detecciones nuevas sin personId permanecen editables en el bloque sin contaminar el directorio.';
