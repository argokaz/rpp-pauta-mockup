...
export function OperationsAdmin({ canManageUsers, getAccessToken, initialDate, onClose, onWorkspaceChange, repository, workspace }: OperationsAdminProps) {
  ...
  const [invite, setInvite] = useState({ email: "", fullName: "", role: "producer" as EditorialUser["role"], programIds: [] as string[] });
  ...
  async function inviteUser() {
    if (!getAccessToken) { setMessage("Las invitaciones solo están disponibles con Supabase."); return; }
    setSaving(true); setMessage("");
    try {
      const response = await fetch("/api/admin/users", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${await getAccessToken()}` }, body: JSON.stringify(invite) });
      const body = await response.json() as { error?: string };
      if (!response.ok) throw new Error(body.error || "No se pudo enviar la invitación.");
      setInvite({ email: "", fullName: "", role: "producer", programIds: [] });
      setUsers(await repository.loadEditorialUsers?.() ?? users);
      setMessage("Invitación enviada. La persona definirá su contraseña desde el correo.");
    ...
    <div className="modal-backdrop operations-admin-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal operations-admin" role="dialog" aria-modal="true" aria-labelledby="operations-admin-title">
        ...
        <nav aria-label="Secciones administrativas">
          ...
          {canManageUsers && <button className={tab === "team" ? "active" : ""} onClick={() => setTab("team")}>Equipo y permisos</button>}
        </nav>
        ...
        {tab === "team" && <div className="admin-section team-admin">
          <section className="invite-card"><header><strong>Invitar a una persona</strong><span>Recibirá un correo para definir su acceso.</span></header><div><label><span>Nombre</span><input value={invite.fullName} onChange={(event) => setInvite({ ...invite, fullName: event.target.value })} /></label><label><span>Correo</span><input type="email" value={invite.email} onChange={(event) => setInvite({ ...invite, email: event.target.value })} /></label><label><span>Rol</span><select value={invite.role} onChange={(event) => setInvite({ ...invite, role: event.target.value as EditorialUser["role"] })}>{roles.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}</select></label>{["producer", "viewer"].includes(invite.role) && <div className="program-checks">{programs.filter((program) => program.managed).map((program) => <label key={program.id}><input type="checkbox" checked={invite.programIds.includes(program.id)} onChange={() => toggleProgram(program.id, invite.programIds, (programIds) => setInvite({ ...invite, programIds }))} />{program.shortName}</label>)}</div>}<button className="primary" disabled={saving || !invite.email || !invite.fullName} onClick={() => void inviteUser()}>Enviar invitación</button></div></section>
          <section className="team-list"><header><strong>Usuarios existentes</strong><span>{loadingUsers ? "Cargando..." : `${users.length} cuentas`}</span></header><div>{users.map((user) => <button key={user.id} onClick={() => setUserDraft(user)}><span><strong>{user.fullName || user.email}</strong><small>{user.email}</small></span><b>{roles.find((role) => role.value === user.role)?.label}</b><em>{user.active ? "Activo" : "Suspendido"}</em></button>)}</div></section>
        </div>}
