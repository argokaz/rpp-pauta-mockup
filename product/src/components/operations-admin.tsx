...
function blankProgram(): Program {
  return { id: "", name: "", shortName: "", hosts: "", managed: true, active: true, accentColor: "#e21d2f" };
}
      ...
      <section className="modal operations-admin" role="dialog" aria-modal="true" aria-labelledby="operations-admin-title">
        ...
        {tab === "schedule" && <div className="admin-section schedule-admin">
          ...
          <div className="admin-schedule-list">{daySlots.map((slot) => {
            ...
            return <article key={slot.id} className={valid ? "current" : "historical"}><time>{slot.startTime}<span>{slot.endTime}</span></time><div><strong>{program?.shortName ?? slot.programId}</strong><small>{slot.effectiveFrom} a {slot.effectiveTo || "sin fecha de fin"}</small></div><b>{valid ? "Vigente" : "Histórico"}</b><div className="admin-schedule-actions"><button disabled={!slot.active} onClick={() => setSlotDraft({ ...slot, id: `version-${slot.id}`, effectiveFrom: scheduleDate, effectiveTo: null, active: true })}>Crear cambio</button><button disabled={!slot.active || saving} onClick={() => void retireSlot(slot.id)}>Retirar</button></div></article>})}{!daySlots.length && <div className="empty-state compact"><strong>Sin horarios configurados</strong><p>Añade el primer bloque para este día de la semana.</p></div>}</div>
        </div>}
        ...
        {tab === "programs" && <div className="admin-section programs-admin">
          ...
          <div>{[...workspace.programs].sort((a, b) => a.name.localeCompare(b.name, "es")).map((program) => <button key={program.id} onClick={() => setProgramDraft(program)}><i className="admin-program-swatch" style={{ background: program.accentColor }} /><span><strong>{program.name}</strong><small>{program.hosts || "Conductores por completar"}</small></span><b>{program.active ? program.managed ? "Administrado" : "Solo horario" : "Inactivo"}</b></button>)}</div>
        </div>}
        ...
        {programDraft && <div className="admin-drawer"><header><strong>{programDraft.id ? "Editar programa" : "Nuevo programa"}</strong><button onClick={() => setProgramDraft(null)}>Cerrar</button></header><div><label><span>Nombre completo</span><input value={programDraft.name} onChange={(event) => setProgramDraft({ ...programDraft, name: event.target.value })} /></label><label><span>Nombre corto</span><input value={programDraft.shortName} onChange={(event) => setProgramDraft({ ...programDraft, shortName: event.target.value })} /></label><label><span>Conductores</span><input value={programDraft.hosts} onChange={(event) => setProgramDraft({ ...programDraft, hosts: event.target.value })} /></label><label className="admin-color-field"><span>Color de identidad</span><span><input type="color" value={programDraft.accentColor} onChange={(event) => setProgramDraft({ ...programDraft, accentColor: event.target.value })} /><input value={programDraft.accentColor} pattern="#[0-9a-fA-F]{6}" onChange={(event) => setProgramDraft({ ...programDraft, accentColor: event.target.value })} /></span><small>Se usa como acento en la interfaz de producción. Podrás cambiarlo cuando quieras.</small></label><label className="admin-switch"><input type="checkbox" checked={programDraft.managed} onChange={(event) => setProgramDraft({ ...programDraft, managed: event.target.checked })} /><span>Se administra dentro de la herramienta</span></label><label className="admin-switch"><input type="checkbox" checked={programDraft.active} onChange={(event) => setProgramDraft({ ...programDraft, active: event.target.checked })} /><span>Programa activo en la señal</span></label><button className="primary" disabled={saving} onClick={() => void saveProgram()}>Guardar programa</button></div></div>}
