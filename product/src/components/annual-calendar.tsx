  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal annual-calendar-modal" role="dialog" aria-modal="true" aria-labelledby="annual-calendar-title">
        ...
        <div className="annual-calendar-layout">
          <div className="annual-calendar-main">
            ...
            <div className="annual-grid">
              {dates.map((date) => <article key={date.value} className={`${date.outside ? "outside" : ""} ${date.value === selectedDate ? "selected" : ""} ${date.important.some((item) => item.category === "holiday") ? "has-holiday" : ""}`} onClick={() => chooseDate(date.value)}>
                ...
                <div className={`annual-day-events ${date.value === selectedDate && date.important.length === 0 && canEdit ? "has-add-event" : ""}`}>{date.important.slice(0, 2).map((item) => <button className={item.category === "holiday" ? "holiday" : ""} key={item.id} onClick={() => { chooseDate(date.value); onEditImportantDate(item); }}>{item.title}</button>)}{date.important.length > 2 && <button onClick={() => chooseDate(date.value)}>+{date.important.length - 2} más</button>}{date.value === selectedDate && date.important.length === 0 && canEdit && <button className="add-event" onClick={() => onCreateImportantDate(date.value)}>+ Añadir evento</button>}</div>
              </article>)}
            </div>
          </div>
        ...
        <footer><span>Selecciona un día vacío y pulsa “Añadir evento” para abrir el formulario.</span><button onClick={() => chooseDate(isoDate(new Date()))}>Ir a hoy</button></footer>
      </section>
    </div>
  );
}
