        ...
        {proposal.blocks.map((block, index) => {
          ...
          return <details key={`${block.matchedSegmentId || "new"}-${index}`} data-match={block.matchStatus}>
            ...
            <div>
              ...
              <label className="post-ai-result"><span>Resultado</span><select value={block.disposition} onChange={(event) => updateBlock(index, { disposition: event.target.value as PostComparisonBlock["disposition"] })}><option value="unconfirmed">Sin confirmar</option><option value="aired">Emitido</option><option value="partial">Parcial</option><option value="skipped">No salió</option><option value="added_live">Añadido en vivo</option></select></label>
              <label className="post-ai-format"><span>Formato</span><select value={block.type} onChange={(event) => updateBlock(index, { type: event.target.value as PostComparisonBlock["type"] })}>{Object.entries(typeLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
              ...
              <label className="post-ai-sequence"><span>Secuencia</span><input value={block.sequence} onChange={(event) => updateBlock(index, { sequence: event.target.value })} placeholder="Nombre de la secuencia, si existe" /></label>
